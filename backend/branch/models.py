...
class MarketFeature2(Market):
    info = models.OneToOneField(MarketFeature1, on_delete=models.SET_NULL, related_name='market_one', null=True,
                                blank=True)
...
class MarketManagers(models.Model):  #جدول مربوط به مسئول مارکت ها 
    ...
    user_type = models.CharField(choices=TYPES, null=True, blank=True, max_length=5) #انواع کاربر مرکز داریم که شما فقط با مسئول مارکت اول که تایپش mco است کار دارید. برای ارسال پیامک و غیره
    market = models.ManyToManyField(MarketFeature2, blank=True, related_name='users')
